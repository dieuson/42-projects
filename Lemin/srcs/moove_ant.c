...
int 		set_to_the_start(char **route, t_check *check, char **good_roads, t_ref *ref)
{
	t_cells *tmp;
	char 	**first_cell;
	...
	tmp = check->start_list;
	while (ligne <= calc_last_ligne(check, ref) && good_roads[ligne] 
...
int 		moove_ants(t_check *check, t_ref *ref, char **good_roads)
{
	...
	while (verif_someone || ref->nb_ants_ref <= check->ants)
	{
		...
		if (ref->nb_ants_ref <= check->ants)
			set_to_the_start(&route, check, good_roads, ref);
